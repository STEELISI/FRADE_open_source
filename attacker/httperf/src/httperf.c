...
static struct option longopts[] = {
	...
	{"multiaddress", required_argument, (int *) &param.multiaddress, 0},
	...
	{"wlist", required_argument, (int *) &param.wlist, 0},
	{0, 0, 0, 0}
};
...
usage(void)
{
	printf("Usage: %s "
	       ...
	       "\t[--max-piped-calls N] [--method S] [--multiaddress S] [--no-host-hdr]\n"
...
{
        extern Load_Generator uri_fixed, uri_wlog, uri_wset, uri_wlist, conn_rate, call_seq;
	...
	while ((ch =
		...
		case 0:
			...
			} else if (flag == &param.multiaddress)
			  {
				param.multiaddress = optarg;
				printf("Multiaddress %s\n", param.multiaddress);
			...
			} else if (flag == &param.wlist) {
				gen[1] = &uri_wlist;
                                param.wlist.file = optarg;
				printf("File is %s\n",param.wlist.file);
			}
	...
	if (param.multiaddress)
		printf(" --multiaddress=%s", param.multiaddress);
	if (param.rate.rate_param > 0.0) {
		switch (param.rate.dist) {
		case DETERMINISTIC:
			/*
