  ...
  if(semNodes.find(*end) != semNodes.end())
    {
      ...
      string s2 ("hit.json");
      string s3(*end);
    /*  if (start->uri.find(s2) != std::string::npos && s3.find(s2) != std::string::npos) {
         cout << endl<<"found hit.json!" << '\n';
         cout << "Transition not found for " << ud->ip << endl;
         ud->endNode = semNodes.at(*end);
         return 0.1;
     } */
      if (start->uri.find(s2) != std::string::npos || s3.find(s2) != std::string::npos) {
         cout << endl<<"One found hit.json!" << '\n';
         cout << "Transition not found for " << ud->ip << endl;
         ud->endNode = semNodes.at(*end);
         return 0.01;
     }
...
//  if(req_count%10 == 1)
  ...
  catch (const out_of_range e) 
  {
    ...
    if (isMainRequest(requestedFile)){
      ...
      cout<<endl<<numWindows<<" "<<userIP<<" "<< reqTime<<" "<<endl;
      if(count_main % 5 == 0){
    ...
    cout<<endl<<userIP<<endl;
    //return;
  }
  cout<<endl<<"Size of "<< sizeof(SemNode)<<endl;
  #ifdef DEBUG
...
/*---------Deception Part End-------------*/
 ...
 if (isMainRequest(requestedFile) && (count_main % 5 == 0)) {
 //if (isMainRequest(requestedFile)) {
      ud->seqLength += 1;
      #ifdef DEBUG
      cerr << "Got a main request: " << *requestedFile << endl;
      #endif
      cout<<endl<<"Main Req"<< *requestedFile << endl;
