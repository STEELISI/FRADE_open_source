...
long start_req =0;
long end_req =0;
std::set<std::string> embedded_reqs;
std::string* any = new std::string("any");
//Wikipedia
...
void FRADE::configure(string config_file, int modules) {
  ...
  //cout<<endl<<modules<<" Modules "<<endl;
  if (!cfile) {
  ...
  string dyn_conf_file, sem_conf_file, windows_string, folders_string, main_reqs_string, dummy_links_file, embedded_req_file, group_file = "";
  try {
  ...
  try {
    embedded_req_file = path + config.at("EMBEDDED_REQ_FILE");
  } catch (const out_of_range e) {
    cerr << "EMBEDDED_REQ_FILE not set. Set it in " << config_file << ".\n";
    exit(1);
  }
  ...
  string eline;
   ifstream erfile(embedded_req_file);
  while(getline(erfile,eline)) {
     embedded_reqs.insert(eline);    
  }
  erfile.close();
...
double FRADE::getTransitionProb2(UserData *ud, string *end) {
  ...
  prob = start->transTo(any, &endNode);
  if (prob > 0)
    {

     double p = start->transTo(end, &endNode);
      cout <<"ANy  Found exact transition to " << end << " returning " << prob << " for " << ud->ip << endl;
      ud->endNode = endNode;
      ud->prevFile = &endNode->uri;
      return prob;
    }
 ...
 for(string s : startList) 
  cout << "aaa startList " << s << " for " << ud->ip << endl;
  for(string e : endList) 
   cout << "aaa endList " << e << " for " << ud->ip << endl;
...
bool FRADE::dynamics1and3(UserData *ud, unsigned long reqTime, string *reqfile) {
  ...
  for (int i = 0; i < numWindows; i++) {
    ...
    if (reqTime - ud->times[i] >= windows[i]) {
      ...
      //cout<<" "<<ud->procs[i]<<" At ONE "<<dyn3Threshes[i]<<" "<<*reqfile<<" i:"<<i<<" ip:"<<ud->ip << endl;
    }
    else {
        ud->counts[i] += 1;
        ud->procs[i] += time_to_process;  //proc_time[*reqfile];
	// Is Dyn1 on?
        ...
        //cout<<" "<<ud->procs[i]<<" At 3 "<<dyn3Threshes[i]<<" "<<*reqfile<<" i:"<<i<<" ip:"<<ud->ip << "  "<< activemodules <<endl;
...
bool FRADE::dynamics2(UserData *ud, unsigned long reqTime, string *reqfile){
              ...
              tok.clear();
              split(*reqfile,'?',tok);
              //cout << "Found Embedded Req" <<endl;

        if(find(ud->current_list.begin(),ud->current_list.end(),*reqfile) != ud->current_list.end() || find(ud->current_list.begin(),ud->current_list.end(),tok[0]) != ud->current_list.end())
		...
		//cout << "***Not in the list, Considering it main" << endl;
                /*for (int i = 0; i < numWindows; i++) {
                        if(ud->embedtimes[i] == 0){
                                ud->embedcounts[i] = 1;
                                ud->embedtimes[i] = reqTime;
			...
			cout <<endl<<ud->embedcounts[i] <<" "<<dyn2Threshes[i]<<endl;
               ...
               //If not find in the AOL instead of blacklisting immediately, we can consider such a request to be a main request
               // Like a user can also request for an image/file as a main request
               ud->seqLength += 1;
               #ifdef DEBUG
               cerr << "Got a not in AOL main request: " << *reqfile << endl;
               #endif
              ...
              if (dynamics1and3(ud, reqTime,reqfile)){
            ...
            #endif
            if (semantics(ud, reqfile)) {
              struct timeval t;
              gettimeofday (&t, NULL);
              cout << "Blacklist sem " << ud->ip << " time: " << t.tv_sec<<endl;
              blacklistIP(ud->ip);
          } 
            /*----------------Semantics End---------------*/
          }
...
/*
bool FRADE::isMainRequest_Old(string *uri) {
...
bool FRADE::isMainRequest(string *uri) {
    ...
    /* If request is not in the embedded requests file. We can assume it to be main. */


    if (!(embedded_reqs.count(*uri))) {
      //cout<<endl<<"Found Main";
      return true;
    }
 vector<string> tok;
 tok.clear();
 split(*uri,'?',tok);
    if (!(embedded_reqs.count(tok[0]))) {
      cout<<endl<<"Found Main";
      return true;
    }


  cout<<endl<<"Embedded"<<*uri<<" "<<tok[0];
  return false;
}
