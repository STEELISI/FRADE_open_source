...
parser.add_argument('--minip', action='store_true', default=False,  help='Use the minimum number of IPs to maintain the sessions. (False by default.)')
...
class HTTPClient(myasyncore.dispatcher):
    ...
    def handle_close(self, inactivity_timeout=False):
        ...
        if inactivity_timeout and self.debug:
    ...
    def handle_write(self):
        ...
        if sent >= self.req_size and self.debug:
...
def get_ips():
    ...
    for dev in devs:
        ...
        if args.minip and len(ips) > args.num_sessions  and args.num_sessions > 0:
            break
...
def main():
    ...
    eb = EventBank(args.num_sessions, s, ips, timeout=int(args.timeout)) 
