...
cmd="cd ~/frade/experiments/run; sudo bash start_detector.sh $modules $m4"
echo $cmd
ssh  -o StrictHostKeyChecking=no $1.$EXP.frade "$cmd" &
sleep 15
echo "Starting leg traffic"
ssh -o StrictHostKeyChecking=no  attacker0.$EXP.frade "sudo python3.4 ~/frade/traffic/smart_attacker/legitimate.py -s $1 --sessions 100 --logs /proj/FRADE/MTurk/Normalized-logs/$1-new.log &" &
echo "Sleeping"
sleep $INT
j=1
while [ $j -le $3 ] ; do
